...
class SubjectRdnOneAvaTest {
    ...
    void passTest() throws Exception {
        X509Certificate certificate = createTestCertificate(caExtension.getCaPublicKey(), caExtension.getCaPrivateKey(), caExtension.getIsserDN(),
                new X500Name("CN=Common Name, O=Lint, C=DE"));
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SubjectRdnOneAva(), certificate);
    }

    @Test
    void errorTest() throws Exception {
        X509Certificate certificate = createTestCertificate(caExtension.getCaPublicKey(), caExtension.getCaPrivateKey(), caExtension.getIsserDN(),
                new X500Name("CN=Certificate+OU=Lint, O=Lint, C=DE"));
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SubjectRdnOneAva(), certificate);
    }

    @Test
    void notApplicableTest() throws Exception {
        X509Certificate certificate = caExtension.getCaCertificate();
        caExtension.assertLintResult(LintResult.of(Status.NA), new SubjectRdnOneAva(), certificate);
    }
