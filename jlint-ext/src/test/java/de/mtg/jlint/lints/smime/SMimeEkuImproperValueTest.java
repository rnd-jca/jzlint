...
class SMimeEkuImproperValueTest {
    ...
    void passTest() throws Exception {
        ...
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SMimeEkuImproperValue(), certificate);
    }

    @Test
    void errorTest() throws Exception {
        X509Certificate certificate = createWrongCertificate();
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SMimeEkuImproperValue(), certificate);
    }

    @Test
    void notEffectiveTest() throws Exception {
        X509Certificate certificate = caExtension.createNotEffectiveSmimeCertificate();
        caExtension.assertLintResult(LintResult.of(Status.NE), new SMimeEkuImproperValue(), certificate);
    }

    @Test
    void notApplicableTest() throws Exception {
        X509Certificate certificate = caExtension.getCaCertificate();
        caExtension.assertLintResult(LintResult.of(Status.NA), new SMimeEkuImproperValue(), certificate);
    }

    private X509Certificate createWrongCertificate() throws NoSuchAlgorithmException, IOException, OperatorCreationException, CertificateException, NoSuchProviderException {
