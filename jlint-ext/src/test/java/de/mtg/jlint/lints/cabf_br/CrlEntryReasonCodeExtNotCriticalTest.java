...
class CrlEntryReasonCodeExtNotCriticalTest {
    ...
    void naTest() throws NoSuchAlgorithmException, IOException, OperatorCreationException, CRLException {
        ...
        caExtension.assertLintResult(LintResult.of(Status.NA), false, new CrlEntryReasonCodeExtNotCritical(), crl, null);
    }
    ...
    void passTest() throws NoSuchAlgorithmException, IOException, OperatorCreationException, CRLException {
        ...
        caExtension.assertLintResult(LintResult.of(Status.PASS), true, new CrlEntryReasonCodeExtNotCritical(), crl, null);
    }

    @Test
    void errorTest() throws CRLException, OperatorCreationException, IOException {
        X509CRL crl = createTestCRL(true);
        caExtension.assertLintResult(LintResult.of(Status.ERROR), true, new CrlEntryReasonCodeExtNotCritical(), crl, null);
    }
    ...
    private X509CRL createTestCRL(boolean critical) throws CRLException, OperatorCreationException, IOException {
        ...
        Extension firstReasonCode = new Extension(Extension.reasonCode, critical, crlReason.toASN1Primitive().getEncoded(ASN1Encoding.DER));
        Extensions firstExtensions = new Extensions(firstReasonCode);
        Extension secondReasonCode = new Extension(Extension.reasonCode, false, crlReason.toASN1Primitive().getEncoded(ASN1Encoding.DER));
