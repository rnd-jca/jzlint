...
class SmimeAiaPresentTest {
    ...
    void passTest() throws Exception {
        ...
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SmimeAiaPresent(), certificate);
    }
    ...
    void warnTest() throws Exception {
        ...
        caExtension.assertLintResult(LintResult.of(Status.WARN), new SmimeAiaPresent(), certificate);
    }

    @Test
    void notApplicableTest() throws Exception {
        X509Certificate certificate = caExtension.getCaCertificate();
        caExtension.assertLintResult(LintResult.of(Status.NA), new SmimeAiaPresent(), certificate);
    }
    ...
    public X509Certificate createTestCertificate(PublicKey caPublicKey, PrivateKey caPrivateKey, X500Name caIssuerDN) throws NoSuchAlgorithmException, IOException, OperatorCreationException, CertificateException, NoSuchProviderException, SignatureException, InvalidKeyException {
        ...
        Extension aia = new Extension(Extension.authorityInfoAccess, false, authorityInformationAccess.toASN1Primitive().getEncoded(ASN1Encoding.DER));
