...
class SMimeSANPresentTest {
    ...
    void passTest() throws Exception {
        ...
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SMimeSANPresent(), certificate);
    }

    @Test
    void errorTest() throws Exception {
        X509Certificate certificate = caExtension.createSMimeEECertificate();
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SMimeSANPresent(), certificate);
    }

    @Test
    void notApplicableTest() throws Exception {
        X509Certificate certificate = caExtension.getCaCertificate();
        caExtension.assertLintResult(LintResult.of(Status.NA), new SMimeSANPresent(), certificate);
    }
    ...
    public X509Certificate createTestCertificate(PrivateKey caPrivateKey, X500Name caIssuerDN) throws NoSuchAlgorithmException, IOException, OperatorCreationException, CertificateException, NoSuchProviderException, SignatureException, InvalidKeyException {
        ...
        GeneralName generalName = new GeneralName(GeneralName.rfc822Name, "test@example.com");
        ...
        Extension san = new Extension(Extension.subjectAlternativeName, false, generalNames.toASN1Primitive().getEncoded(ASN1Encoding.DER));
