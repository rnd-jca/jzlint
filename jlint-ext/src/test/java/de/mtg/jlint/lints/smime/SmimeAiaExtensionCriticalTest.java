...
class SmimeAiaExtensionCriticalTest {
    ...
    void passTest() throws Exception {
        ...
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SmimeAiaExtensionCritical(), certificate);
    }
    ...
    void errorTest() throws Exception {
        ...
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SmimeAiaExtensionCritical(), certificate);
    }

    @Test
    void notApplicableTest() throws Exception {
        X509Certificate certificate = caExtension.getCaCertificate();
        caExtension.assertLintResult(LintResult.of(Status.NA), new SmimeAiaExtensionCritical(), certificate);
        certificate = caExtension.createSMimeEECertificate();
        caExtension.assertLintResult(LintResult.of(Status.NA), new SmimeAiaExtensionCritical(), certificate);
    }

    public X509Certificate createTestCertificate(PublicKey caPublicKey, PrivateKey caPrivateKey, X500Name caIssuerDN, boolean critical) throws NoSuchAlgorithmException, IOException, OperatorCreationException, CertificateException, NoSuchProviderException, SignatureException, InvalidKeyException {
        ...
        Extension aia = new Extension(Extension.authorityInfoAccess, critical, authorityInformationAccess.toASN1Primitive().getEncoded(ASN1Encoding.DER));
