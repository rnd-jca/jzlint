...
class SmimeCrldpContainsUriFullnameTest {
    ...
    void passTest() throws Exception {
        X509Certificate certificate = createTestCertificate(caExtension.getCaPublicKey(), caExtension.getCaPrivateKey(), caExtension.getIsserDN(), 6);
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SmimeCrldpContainsUriFullname(), certificate);
    }

    @Test
    void errorTest() throws Exception {
        X509Certificate certificate = createTestCertificate(caExtension.getCaPublicKey(), caExtension.getCaPrivateKey(), caExtension.getIsserDN(), 1);
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SmimeCrldpContainsUriFullname(), certificate);
    }

    @Test
    void notApplicableTest() throws Exception {
        X509Certificate certificate = caExtension.getCaCertificate();
        caExtension.assertLintResult(LintResult.of(Status.NA), new SmimeCrldpContainsUriFullname(), certificate);
    }

    private X509Certificate createTestCertificate(PublicKey caPublicKey, PrivateKey caPrivateKey, X500Name caIssuerDN, int type) throws NoSuchAlgorithmException, IOException, OperatorCreationException, CertificateException, NoSuchProviderException {
    ...
    private static Extension getCRLDPs(int type) throws IOException {
        ...
        return new Extension(Extension.cRLDistributionPoints, false, crlDistributionPoint.toASN1Primitive().getEncoded(ASN1Encoding.DER));
    }
