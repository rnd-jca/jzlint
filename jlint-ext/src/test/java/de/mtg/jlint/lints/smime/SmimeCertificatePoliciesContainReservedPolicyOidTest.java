...
class SmimeCertificatePoliciesContainReservedPolicyOidTest {
    ...
    void passTest() throws Exception {
        X509Certificate certificate = createTestCertificate(caExtension.getCaPublicKey(), caExtension.getCaPrivateKey(), caExtension.getIsserDN(), Arrays.asList("2.23.140.1.5.1.3", "1.2.3.4"));
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SmimeCertificatePoliciesContainReservedPolicyOid(), certificate);
    }

    @Test
    void errorTest() throws Exception {
        X509Certificate certificate = createTestCertificate(caExtension.getCaPublicKey(), caExtension.getCaPrivateKey(), caExtension.getIsserDN(), Arrays.asList("2.23.140.1.5.1.2", "2.23.140.1.5.1.3"));
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SmimeCertificatePoliciesContainReservedPolicyOid(), certificate);
    }
    ...
    private X509Certificate createTestCertificate(PublicKey caPublicKey, PrivateKey caPrivateKey, X500Name caIssuerDN, List<String> oids) throws NoSuchAlgorithmException, IOException, OperatorCreationException, CertificateException, NoSuchProviderException {
        ...
        Optional<Extension> certificatePolicies = getCertificatePolicies(oids);
        if (certificatePolicies.isPresent()) {
