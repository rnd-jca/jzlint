...
class SMimeEkuMailProtectionMissingTest {
    ...
    void passTest() throws Exception {
        ...
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SMimeEkuMailProtectionMissing(), certificate);
    }

    @Test
    void errorTest() throws Exception {
        X509Certificate certificate = createWrongCertificate(true);
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SMimeEkuMailProtectionMissing(), certificate);
    }

    @Test
    void notEffectiveTest() throws Exception {
        X509Certificate certificate = caExtension.createNotEffectiveSmimeCertificate();
        caExtension.assertLintResult(LintResult.of(Status.NE), new SMimeEkuMailProtectionMissing(), certificate);
    }

    @Test
    void notApplicableTest() throws Exception {
        X509Certificate certificate = createWrongCertificate(false);
        caExtension.assertLintResult(LintResult.of(Status.NA), new SMimeEkuMailProtectionMissing(), certificate);
    }

    private X509Certificate createWrongCertificate(boolean withEKU) throws NoSuchAlgorithmException, IOException, OperatorCreationException, CertificateException, NoSuchProviderException {
