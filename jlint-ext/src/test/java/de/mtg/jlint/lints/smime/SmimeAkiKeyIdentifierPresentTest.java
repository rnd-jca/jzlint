...
class SmimeAkiKeyIdentifierPresentTest {
    ...
    void passTest() throws Exception {
        ...
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SmimeAkiKeyIdentifierPresent(), certificate);
    }

    @Test
    void errorTest() throws Exception {
        X509Certificate certificate = createTestCertificate(caExtension.getCaPrivateKey(), caExtension.getIsserDN());
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SmimeAkiKeyIdentifierPresent(), certificate);
    }

    @Test
    void notApplicableTest() throws Exception {
        X509Certificate certificate = caExtension.getCaCertificate();
        caExtension.assertLintResult(LintResult.of(Status.NA), new SmimeAkiKeyIdentifierPresent(), certificate);
    }

    public X509Certificate createTestCertificate(PrivateKey caPrivateKey, X500Name caIssuerDN) throws NoSuchAlgorithmException, IOException, OperatorCreationException, CertificateException, NoSuchProviderException, SignatureException, InvalidKeyException {
        ...
        AuthorityKeyIdentifier aki = new AuthorityKeyIdentifier(null, BigInteger.ONE);
