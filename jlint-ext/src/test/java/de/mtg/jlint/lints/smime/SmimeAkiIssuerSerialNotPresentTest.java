...
class SmimeAkiIssuerSerialNotPresentTest {
    ...
    void passTest() throws Exception {
        X509Certificate certificate = caExtension.createSMimeEECertificate();
        caExtension.assertLintResult(LintResult.of(Status.PASS), new SmimeAkiIssuerSerialNotPresent(), certificate);
    }

    @Test
    void errorTest() throws Exception {
        X509Certificate certificate = createTestCertificate(caExtension.getCaPrivateKey(), caExtension.getIsserDN(), new GeneralNames(new GeneralName(GeneralName.directoryName, "CN=Issuer")), null);
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SmimeAkiIssuerSerialNotPresent(), certificate);
        certificate = createTestCertificate(caExtension.getCaPrivateKey(), caExtension.getIsserDN(), null, BigInteger.ONE);
        caExtension.assertLintResult(LintResult.of(Status.ERROR), new SmimeAkiIssuerSerialNotPresent(), certificate);
    }
    ...
    void notApplicableTest() throws Exception {
        ...
        caExtension.assertLintResult(LintResult.of(Status.NA), new SmimeAkiIssuerSerialNotPresent(), certificate);
    }

    public X509Certificate createTestCertificate(PrivateKey caPrivateKey, X500Name caIssuerDN, GeneralNames generalNames, BigInteger serial) throws NoSuchAlgorithmException, IOException, OperatorCreationException, CertificateException, NoSuchProviderException, SignatureException, InvalidKeyException {
        ...
        AuthorityKeyIdentifier aki = new AuthorityKeyIdentifier(generalNames, serial);
        SubjectKeyIdentifier ski = new JcaX509ExtensionUtils().createSubjectKeyIdentifier(publicKey);
        Extension akie = new Extension(Extension.authorityKeyIdentifier, true, aki.toASN1Primitive().getEncoded(ASN1Encoding.DER));
