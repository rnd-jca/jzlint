...
public class SubCertStreetAddressShouldNotExist implements JavaLint {
    ...
    public LintResult execute(X509Certificate certificate) {
        ...
        if (organization.isEmpty() && givenName.isEmpty() && surname.isEmpty()) {
            List<AttributeTypeAndValue> street = Utils.getSubjectDNNameComponent(certificate, BCStyle.STREET.getId());
            if (!street.isEmpty()) {
