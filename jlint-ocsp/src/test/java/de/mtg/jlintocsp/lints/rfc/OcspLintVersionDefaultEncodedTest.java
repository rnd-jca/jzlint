...
class OcspLintVersionDefaultEncodedTest {
    ...
    void passTest() {
        ...
        caExtension.assertLintResult(LintResult.of(Status.PASS), true, new OcspLintVersionDefaultEncoded(), correctResponse);
    }
    ...
    void errorTest() throws NoSuchAlgorithmException, IOException, SignatureException, NoSuchProviderException, InvalidKeyException {
        ...
        caExtension.assertLintResult(LintResult.of(Status.ERROR), true, new OcspLintVersionDefaultEncoded(), response);
    }

    public byte[] createWrongOCSPResponse()
        ...
        CertID certID = new CertID(aid, new DEROctetString(issuerDNHash), new DEROctetString(issuerPKHash),
                new ASN1Integer(caExtension.getCaCertificate().getSerialNumber()));
        ...
        ASN1EncodableVector responseDataVector = new ASN1EncodableVector();
        responseDataVector.add(new DERTaggedObject(true, 0, new ASN1Integer(0L)));
        responseDataVector.add(responderID);
        responseDataVector.add(producedAt);
        responseDataVector.add(new DERSequence(vector));
        ResponseData responseData = ResponseData.getInstance(new DERSequence(responseDataVector));
        ...
        ResponseBytes responseBytes = new ResponseBytes(new ASN1ObjectIdentifier("1.3.6.1.5.5.7.48.1.1"),
                new DEROctetString(basicOCSPResponse.getEncoded(ASN1Encoding.DER)));
